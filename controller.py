...
#             if response is None:
#                 raise RuntimeError("Final response LLM returned no
# response.")
...
#         return ("I was unable to generate a response for the requested
# task.")
...
class Controller:
    ...
    def _build_rag(self):
        """
        Builds the complete RAG system: one path for ingestion (adding
        documents), the other for retrieval.
        """
        embedder = Embedder()
        vector_store = VectorStore()
        retriever = Retriever(embedder=embedder, vector_store=vector_store,
    ...
    def run(self, state):
        ...
        for _ in range(self.max_steps):
            ...
            if agent_name not in self.agents:
                # CHANGED: was two adjacent string literals
                # (f"...by" "router: " f"{agent_name}") that
                # concatenated into "...byrouter: <name>" with a
                # missing space -- purely cosmetic, but confusing in
                # error logs. Now a single f-string.
                state.add_error({"component": "controller",
                                 "error": (f"Unknown agent selected by "
                                           f"router: {agent_name}")})
                return state
            ...
            try:
                ...
                print("Errors:", state.errors)
                print("=================\n")
                # CHANGED: removed the leftover debug print block that
                # used to sit here. It's harmless but noisy, and it's
                # a straightforward source of confusion left over from
                # active debugging -- swap in your own logging call
                # here if you want step-by-step tracing.

            except Exception as e:
                state.add_error({"agent": agent_name,
                                 "error": str(e)})
                # CHANGED: removed the debug print block that used to
                # sit here too -- it printed "MAX STEPS REACHED" on
                # *every* per-agent exception, not just when max_steps
                # was actually hit, which was actively misleading.
                state.current_agent = None
                return state
    ...
    def _workflow_complete(self, state):
        ...
        Determines whether the requested work has been completed.
        This prevents the Router from having to decide when the
        entire system is finished -- the Controller knows what
        outputs actually exist in state.
        """
        request = state.user_request.lower()
        forecasting_requested = any(
            word in request
        ...
        # CHANGED: this function fell off the end here with no return
        # statement in the "everything required is present" case,
        # which in Python means it implicitly returns None -- falsy.
        # `if self._workflow_complete(state):` in run() could
        # therefore never be True, so the controller never
        # short-circuited to the final response even once all
        # requested work was actually done; it always looped back to
        # the router (relying entirely on the router eventually
        # picking direct_response, or the loop hitting max_steps).
        return True

    def _generate_final_response(self, state):
        """
        Generates the final natural-language answer from the
        completed AgentState. The final LLM does NOT perform the
        analysis itself -- it just turns the specialist agents'
        results into a response for the user.
        """
        context = state.to_dict()
        system_prompt = """
You are the final response generator for a multi-agent
...
"""
    ...
    def _build_fallback_response(self, state):
        """
        Generates a basic response without making another LLM call.
        Useful if the final response LLM hits a rate limit or
        otherwise fails.
        """
        if state.forecast is not None:
            response = "Forecast generated successfully."
