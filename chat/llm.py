...
class LLMClient:
    ...
    def _send_request(self, messages, tools=None):
        ...
        response = self.client.chat.completions.create(
            messages=messages,
            model=self.model,
            # CHANGED: was `tools=tools`. Any agent whose ToolRegistry has
            # zero tools registered (e.g. forecasting_agent, if it ever
            # went through the tool-calling loop) would pass an empty
            # list here, and Groq's API can reject an empty non-null
            # `tools` array. Normalizing empty/falsy to None is safe for
            # every caller.
